...
If any of those prequisites have prerequisites of their own, then they
...
them before forcing everyone else to ugprade.  This is what stacks
are designed for.
...
explictly, then the operation is applied to whichever stack is marked
as the default.
...
alhpanumeric and are forced to lowercase).  If you want to see the
...
Now look at the repostiory contents again.  You'll use the
L<list|App::Pinto::Command::list> commmand, but this time you'll use
...
L<install|App::Pinto::Command::install> cammand and point to to the
right stack:
...
pinned.  Notic every package in the F<URI-1.60.tar.gz> distribution
...
C<prod> that contains your known-good dependncies.  Likewise, you could
...
version of the distribiution called F<URI-1.60_PATCHED.tar.gz>.  You
...
includes your patch or the author has fixeed the bug by other means.
...
C<--pull> option also tells Pinto to pull any prerequsities that it
